...
public class AmountRentalCountDAOImplTest extends TestCase {
    ...
    public void testGetAmountRentalCountData() throws Exception {
        List<DomainWidgetContent> amountRentalCounts = DAOFactory.getInstance().getAmountRentalCountDAO().getAmountRentalCountData();
    ...
    public void testGetInfoMap() throws DBException
    {
        ...
        for (int i = 0; i < amountRentalCounts.size(); i++) {
            try {
                ...
                System.out.println(amountRentalCounts.get(i).getFullDomainInfo().toString());
            }
            catch (Throwable e)
            {
